...
import { CinematicPipeline } from "@/components/dashboard/CinematicPipeline"
...
export default function DashboardOverview() {
  ...
  const startAnalysis = async (id: string) => {
    try {
      await api.startDatasetPipeline(id)
      setStep(2) // Move to Live Processing Workspace
      
      // Also kick off the real analysis so we have the report ready for step 3
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      {/* Dataset Upload / Processing Area */}
      {step === 2 && datasetId ? (
        <CinematicPipeline datasetId={datasetId} onComplete={() => setStep(3)} />
      ) : step === 3 ? (
        <div className="mb-12">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-[var(--fg-base)]">Pipeline Results</h2>
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <span className="text-[var(--brand-light)] font-medium">Processing Complete</span>
            </div>
          </div>
          ...
          <Card className="h-full p-6 flex flex-row gap-6">
            <div className="w-1/3 flex flex-col justify-between border-r border-[var(--color-border)] pr-6">
              <div>
                ...
                <h3 className="text-lg font-semibold text-[var(--fg-base)]">Pipeline Deployed</h3>
                <p className="text-xs text-slate-400 mt-1">Dataset: {datasetId}</p>
                <p className="text-xs text-slate-500 mt-4 leading-relaxed">AgentIQ successfully processed the dataset and trained the final model.</p>
              </div>
              <div className="flex flex-col gap-2 mt-4">
                <a href={datasetId ? api.getDownloadUrl(datasetId) : "#"} download className="block">
                  <Button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white border-none gap-2">
                    <Download size={14} /> Download Cleaned Data
                  </Button>
                </a>
                <Button variant="ghost" size="sm" onClick={() => { setStep(0); setDatasetId(null); setReport(null); }} className="w-full">Process New Dataset</Button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-6">
              ...
              <div>
                <h4 className="text-xs font-semibold text-[var(--fg-base)] mb-3 flex items-center gap-2">
                  <AlertTriangle size={14} className="text-amber-400" /> Issues Addressed by Agents
                </h4>
                <ul className="space-y-2">
                  {report?.problems_found?.map((prob: string, i: number) => (
                  ...
                  {(!report || !report.problems_found?.length) && (
                    <li className="text-xs text-slate-500 italic">No issues recorded.</li>
                  )}
                </ul>
              </div>
              ...
              <div>
                <h4 className="text-xs font-semibold text-[var(--fg-base)] mb-3 flex items-center gap-2">
                  <CheckCircle2 size={14} className="text-emerald-400" /> Transformations Applied
                </h4>
                <ul className="space-y-2">
                  {report?.changes_done?.map((change: string, i: number) => (
                  ...
                  {(!report || !report.changes_done?.length) && (
                    <li className="text-xs text-slate-500 italic">No transformations recorded.</li>
                  )}
                </ul>
              </div>
            </div>
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500">
              ...
              <ChevronRight size={12} className="opacity-50" />
              <span className={step === 3 ? "text-[var(--brand-light)] font-medium" : ""}>3. Results</span>
            </div>
          </div>
              ...
              {step === 1 && (
                <motion.div key="step-1" custom={1} variants={variants} initial="initial" animate="animate" exit="exit" className="absolute inset-0">
                  <Card className="h-full p-8 flex flex-col items-center justify-center">
                    <h3 className="text-xl font-semibold text-[var(--fg-base)] mb-4">Ready for Processing</h3>
                    <p className="text-sm text-slate-400 mb-6 text-center max-w-md">
                      Your dataset has been loaded. Click below to launch the AgentIQ autonomous pipeline and watch the data workforce in action.
                    </p>
                    <Button size="lg" className="bg-[var(--brand-light)] text-black hover:bg-[var(--brand-light)]/90 px-8" onClick={() => startAnalysis(datasetId!)}>
                      START PROCESSING
                    </Button>
                  </Card>
                </motion.div>
